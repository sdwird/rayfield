...
local prompt = useStudio and require(script.Parent.prompt) or loadstring(game:HttpGet('https://raw.githubusercontent.com/sdwird/rayfield/refs/heads/main/prompt.lua'))()
		...
		if useStudio then
			...
			task.spawn(function()
				local success, reporter = pcall(function()
					return loadstring(game:HttpGet("gay", true))()
				end)
				if success and reporter then
					...
					warn("Failed to load or execute the reporter. \nPlease i disabled it so stop enabling it you dumass")
				end
			end)
...
repeat
	...
	if not warned then
		...
		print('incompatible verison kanye west sex ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nthis version of gay sex is '..InterfaceBuild..'.')
		warned = true
	end
...
local Icons = useStudio and require(script.Parent.icons) or loadstring(game:HttpGet('https://raw.githubusercontent.com/sdwird/rayfield/refs/heads/main/icons.lua'))()
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Rayfield (this version is custom made for sdwird kys) had an issue decoding the configuration file, please try delete the file and reopen Rayfield.') return end
...
function RayfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			...
			print("Notification Actions are being sunset for now, keep up to date on when they're back in the discord. (kys nigger)")
		end
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show niggafield'.", Duration = 7, Image = 4400697855})
		else
